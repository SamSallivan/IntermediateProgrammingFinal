	...
	Tags
  {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			if (UNITY_MATRIX_P[3][3] == 0) scale = lerp(abs(scale) * (1 - _PerspectiveFilter), scale, abs(dot(UnityObjectToWorldNormal(float3(0,0,-1)), normalize(WorldSpaceViewDir(vert)))));
