...
SubShader {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
...
#ifdef USE_CURVE
			float4 pos = mul(UNITY_MATRIX_MV, input.vertex);
			float rate = length(pos.x) / _UI_CurveLength;
			float w = pos.w + rate * length(rate);
			pos.yz /= w;
			float4 vPosition = mul(UNITY_MATRIX_P, pos);
#else
			...
			fixed4 faceColor = fixed4(input.color.rgb, opacity) * _FaceColor * _Color;
			faceColor.rgb *= faceColor.a;

			fixed4 outlineColor = _OutlineColor * _Color;
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
			...
			// sample face texture
			half4 faceTexColor = tex2D(_FaceTex, input.faceUV);
			half4 faceColor = input.faceColor * faceTexColor;

			half4 c = faceColor * saturate(d - input.param.w);

			#ifdef OUTLINE_ON
			c = lerp(input.outlineColor, faceColor, saturate(d - input.param.z));
