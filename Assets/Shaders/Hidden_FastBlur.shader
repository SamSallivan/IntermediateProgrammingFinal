...
Shader "Hidden/FastBlur" {
	...
	CGINCLUDE
		...
		struct v2f_ChangeAlpha
		{
			float4 pos : SV_POSITION;
			half2 uv : TEXCOORD0;
		};

		v2f_ChangeAlpha vert4ChangeAlpha(appdata_img v)
		{
			v2f_ChangeAlpha o = (v2f_ChangeAlpha)0;
			o.pos = UnityObjectToClipPos(v.vertex);
			o.uv = v.texcoord;
			return o;
		}

		fixed4 frag4ChangeAlpha(v2f_ChangeAlpha i) : SV_Target
		{
			fixed4 color = tex2D(_MainTex, i.uv);
			return fixed4(color.r, color.g, color.b, color.a);
		}
		...
		fixed4 fragBlur8 ( v2f_withBlurCoords8 i ) : SV_Target
		{
            ...
            fixed4 fragBlur8NoAlpha(v2f_withBlurCoords8 i) : SV_Target
        {
            half2 uv = i.uv.xy;
            half2 netFilterWidth = i.offs;
		...
		fixed4 fragBlurSGX ( v2f_withBlurCoordsSGX i ) : SV_Target
		{
			half2 uv = i.uv.xy;
			
			fixed4 color = tex2D(_MainTex, i.uv) * curve4[3];
			
  			for( int l = 0; l < 3; l++ )  
  			{   
				fixed4 tapA = tex2D(_MainTex, i.offs[l].xy);
				fixed4 tapB = tex2D(_MainTex, i.offs[l].zw); 
	...
	Pass { 
		...
		#pragma vertex vert4ChangeAlpha
		#pragma fragment frag4ChangeAlpha
	...
	Pass {		
		...
		ENDCG
          ...
          Pass{
              ...
              #pragma vertex vertBlurVertical
              #pragma fragment fragBlur8NoAlpha
              #pragma fragmentoption ARB_precision_hint_fastest 
