...
public class EnemyMove : MonoBehaviour, Slappable, Damagable
{
	...
	public Vector3 GetNavMeshPosition(Vector3 pos, float radius = 0.5f)
	{
        NavMeshHit navHit; 
	...
	public void Slap(Vector3 dir)
	{
        ...
        for (int j = 0; j < ragdollColliders.Count; j++)
        {
            ...
            ragdollColliders[j].attachedRigidbody.velocity = tempDir.normalized * ((j == 0) ? 3 : 6);
        }
        ragdollColliders[0].attachedRigidbody.AddForce(Vector3.up * (20f), ForceMode.Impulse);
        // if (lifetime > 0f)
        // {
	...
	{
        materialTimer = 1;
        player.GetComponent<PlayerController>().slamVFX.transform.position = player.GetComponent<PlayerController>().t.position;
        player.GetComponent<PlayerController>().slamVFX.transform.rotation = Quaternion.LookRotation(Vector3.up);
        player.GetComponent<PlayerController>().slamVFX.GetComponent<ParticleSystem>().Play();
        ...
        if (damage.amount < kickMinDamage)
            ...
            if (doll){
				...
				ragdollColliders[0].GetComponent<Rigidbody>().AddForce(Vector3.up * 25f, ForceMode.Impulse);
