...
{
    [System.Serializable]
    public class DistortionUvAnimationBehaviour : BasePlayableBehaviour
    {
        internal DistortionUvAnimationAsset asset;
        ...
        private float totalTilingX = 0;
        private float totalTilingY = 0;
        ...
        internal override void OnAwake(Playable playable)
        {
            ...
            if (sShaderID_MainTexST == 0)
            {
                ...
                sShaderID_MaskTexST = MaterialPropertyHelper.Instance.GetPropertyID("_Mask_ST");
        ...
        internal override void OnEnable()
        {
            ...
            totalTilingX = asset.tillingX;
            totalTilingY = asset.tillingY;
            last_time = 0;
        ...
        internal override void OnUpdate(float currentTime, float deltaTime, Playable playable, FrameData info, object playerData)
        {
            ...
            if (renderer == null || propertyBlock == null || asset == null || asset.speedCurveEnables == null || asset.speedCurveEnables.Length != 4)
            {
                return;
            }
            ...
            if (currentTime - last_time > 0.033f && currentTime - last_time < 30)
            {
                ...
                float speedZ = asset.speedTilingX;
                float speedW = asset.speedTilingY;
                while (t < currentTime)
                {
                    t += 0.033f;
                    if (t > currentTime)
                    ...
                    if (t >= asset.curveBeginTime)
                    {
                        ...
                        if (asset.speedCurveEnables[2] && asset.speedCurveZ != null)
                        {
                            speedZ = asset.speedCurveZ.Evaluate(curveTime);
                        }
                        if (asset.speedCurveEnables[3] && asset.speedCurveW != null)
                        {
                            speedW = asset.speedCurveW.Evaluate(curveTime);
                        }
                    }
                    totalOffsetX += (0.033f * speedX);
                    totalOffsetY += (0.033f * speedY);
                    totalTilingX += (0.033f * speedZ);
                    totalTilingY += (0.033f * speedW);
                    last_time = t;
                }
            }
            else
            {
                ...
                if (currentTime > 0)
                {
                    ...
                    totalTilingX += (deltaTime * speedZ);
                    totalTilingY += (deltaTime * speedW);
                }
            ...
            var st = new Vector4(totalTilingX, totalTilingY, totalOffsetX, totalOffsetY);
            if (asset.isMask)
            {
